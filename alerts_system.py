...
'''
get_alert_info() - функция в которой перечислены метрики, которые мы хотим отслеживать
feed - лента, msg - месседжер
'users_feed' - количество уникальных пользователей (в ленте) - одна из метрик
'users_msg' - количество уникальных пользователей (в месседжере)
'alias' - то что прописываем в запросе для нашей метрики. Например в какой-нибудь формуле будет s_users_feed или s_DAU_feed
'''
...
def get_alert_info():
    ...
    # Хотели отправлять алерты не только для total значений, но и по срезам. Например разбивка по os
    # 'group_levels' - алерты по обоим операционным системам
    # 'formula' - если бы значение os лежало бы не в самой табличке, а в отдельном словаре по ключу user_id, 
    # то могли бы написать примерно так: 'formula': 'dictGetString('название_словаря', 'поле_словаря(os)', user_id(ключ))'
    ...
    # 2021-12-25 18:30:59 -> 18:30
    #  >= '01:00' чтобы корректно работало, т.к. усредняем напрмер по 15-ти минуткам, в начале дня может криво работать
    where_expression_template = ''' 
         toDate(time) in (today(), today() - 1, today() - 7) and formatDateTime(time, '%R') >= '01:00' '''
    
    # Если что-то сломается в метрике users_msg, то будет тегаться определенный user_id
    # Если что-то сломается по срезу os, то будет тегаться определенный user_id
    responsible_users = {
        "metrics" : {"users_msg": "@user_117"},
        "groups" : {"os": "@user_007"}
    }
    
    # Ccылки на графики с изменением метрик
    alert_plots = {
        'total': {
...
# get_select_vars() - функция SELECT
...
# get_from_table_name() - функция FROM
...
# get_where_expression() - функция WHERE
...
# get_group_by_expression() - функция GROUP BY
...
# send_alert() - функция отправляющая алерт
...
# get_alert() - функция которая сходит в базу, соберет данные, в которой напишем запрос
...
# get_metric_alert() - функция в которой логика самих алертов по метрикам
# day_threshhold=0.3 - 30% изменение метрики к предыдущему дню или ко дню неделю назад, оптимальное значение, чтобы замечать отклонения и не спамить
def get_metric_alert(alert_df, metric, metric_alias, slice, group, day_threshhold=0.3):
    ...
    # время начинается с '1970-01-01 00:00:00', 3 часа - это смещение в нашем часовом поясе
...
# global_monitoring() - функция в которую зададим список метрик которые хотим мониторить
def global_monitoring(chat=None):
    chat_id = chat or 904623834  # либо указанный чат либо личнаяпереписка с ботом
    alerts = [
    ...
    # - interval 15 minute, чтобы была самая последняя 15-ти минутка
    ...
    # функция get_alert() будет распаковывыть словарик alerts, доставать информацию по метрикам, формировать запрос, запускать его и получать данные     
